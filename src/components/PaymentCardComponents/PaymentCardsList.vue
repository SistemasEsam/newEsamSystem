...
export default {
  ...
  >
    <template v-slot:top>
      <v-toolbar flat>
        <v-toolbar-title> Lista de cartas </v-toolbar-title>
        <v-divider class="mx-4" inset vertical></v-divider>
        <v-spacer></v-spacer>

        <router-link to="/payment-cards">
          <v-btn color="primary" dark class="mb-2"> Nueva Carta </v-btn>
        </router-link>
      </v-toolbar>
    </template>
    <template v-slot:item.actions="{ item }">
      <v-icon size="small" @click="showDetails(item)"> mdi-eye </v-icon>
