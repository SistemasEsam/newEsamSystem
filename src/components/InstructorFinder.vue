    ...
    <div>
        <div>
            <v-container>
                <v-row>
                    <v-col>
                        <v-text-field v-model="keyWordFilter" label="Ingrese datos a buscar"></v-text-field>
                        <v-radio-group label="Nivel de Educacion" v-model="filterEducation">
                            <v-radio label="Pregrado" id="degree" value="degrees" @change="enableLevelPostDegree(true)">
                            </v-radio>
                            <v-radio label="Postgrado" id="postDegree" value="postDegrees" @change="enableLevelPostDegree(false)">
                            </v-radio>
            ...
            this.instructorList.forEach(async (instructor) => {
                ...
                dataListFiltered.forEach((dataFiltered) => {
                    switch (this.filterEducation) {
                        case 'degrees':
                            if (((dataFiltered.data().careerDegree).toLowerCase()).includes((this.keyWordFilter).toLowerCase())) {
                                this.instructorFilteredList.push(instructor)
                            }
                            break
                        case 'postDegrees':
                            if (((dataFiltered.data().namePostDegree).toLowerCase()).includes((this.keyWordFilter).toLowerCase())) {
                                this.instructorFilteredList.push(instructor)
                            }
                            break
                        case 'courses':
                            if (((dataFiltered.data().nameCourse).toLowerCase()).includes((this.keyWordFilter).toLowerCase())) {
                                this.instructorFilteredList.push(instructor)
                            }
                            break
                    }
